    ...
    $(document).ready(function () {
        $(document).on("click", ".hapus-barang", function () {
            ...
            if (confirm('Yakin ingin menghapus data terpilih?')) {
                $.ajax({
                    url: "{{ route('riwayatbeli.deleteItem') }}",
                    method: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
        ...
        let orderJualIndexURL = "{{ route('orderbeli.index') }}";
        table = $('#dataTable').DataTable({
            ...
            layout: {
                ...
                topEnd: {
                    div: {
                        html: '<a type="button" class="btn btn-primary text-sm btn-sm px-12 py-12 radius-8 d-flex align-items-center gap-2" href="'+ orderJualIndexURL +'"><iconify-icon icon="ic:baseline-plus" class="icon text-xl line-height-1"></iconify-icon>Tambah Pembelian</a>'
                    }
                }
            },
            responsive: true,
            ajax: {
                url: "{{ route('riwayatbeli.data') }}"
            },
