    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body p-24">
                <form action="{{ route('orderjual.checkout') }}" method="post" class="needs-validation" novalidate>
                    @csrf
                    @method('post')
                    <div class="row">
                        <div class="col-12">
...
<script>
    ...
    function updateTotalHarga() {
        $.ajax({
            url: "{{ route('orderjual.totalHarga') }}", // Panggil route yang telah dibuat
            method: "GET",
            success: function (response) {
                $("#totalHarga").text("Rp " + response.total_harga.toLocaleString("id-ID"));
                $("#totalbayar").val(response.total_harga);
                $("#id_pembelian").val(response.id_penjualan.toLocaleString("id-ID"));
            },
            error: function () {
    ...
    $(document).ready(function () {
        ...
        $(document).on("input", ".quantity", function () {
            ...
            $.ajax({
                url: "{{ route('orderjual.updateQuantity') }}",
                method: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
        ...
        $(document).on("input", "#bayar", function () {
            let total = $('#totalbayar').val() || 0;
            let bayar = $(this).val() || 0;
            let kembalian = bayar - total;
            console.log(kembalian);
            if (kembalian < 0) {
                kembalian = 0; // Tidak boleh negatif
            }
    ...
    document.getElementById("submitbyr").addEventListener("click", function (event) {
        ...
        const idPenjualan = document.getElementById("id_pembelian").value;
        if (bayar < totalBayar) {
            ...
            struk(`/sales/struk/${idPenjualan}`, "Struk");        
        }
